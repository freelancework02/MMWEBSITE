...
const QuestionDetail = () => {
  const [menuOpen, setMenuOpen] = useState(false);
    const [takequestion, setTakequestion] = useState([]);
    ...
    const question19 = takequestion.find((q) => q.id === 9);
  ...
  if (!question) return <div className="text-center p-4">لوڈ ہو رہا ہے...</div>;
     ...
     <main className="min-h-screen bg-[#f0f2e6] relative">
            ...
            <div className="max-w-7xl mx-auto flex flex-col md:flex-row gap-6">
              {/* Right content area - Answer details */}
                ...
                >
                  <div className="rtl text-right leading-loose text-[#4a7031] text-lg gulzartext whitespace-pre-wrap">
                    {formatDescription(question.answer)}
                  </div>
                </div>
              ...
              <div className="w-full md:w-1/4 bg-white rounded-xl shadow-sm overflow-hidden border border-green-100 order-2 md:order-1">
                ...
                <div className="p-4 flex flex-col gap-4 rtl cursor-pointer">
                  {takequestion.slice(1, 13).map((q, index) => (
                    <div
                      key={q._id || index}
          ...
          <div className="bg-[#eaf3df] py-10">
            <div className="container mx-auto px-4">
              ...
              {/* Questions Grid */}
              <div className="grid grid-cols-1 sm:grid-cols-2 cursor-pointer md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-6 rtl">
                {takequestion.slice(1, 13).map((q, index) => (
                  <div key={q._id}  onClick={() => navigate(`/question/${q._id || index}`)} className="bg-[#eaf3df] rounded-xl p-4  ">
